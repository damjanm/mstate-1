...
formo <- Surv(Tstart, Tstop, status) ~ match.1 +
  ...
  agecl2.12 + strata(trans)

cfull <- coxph(formo, data = msebmt, method = "breslow")

cx2 <- coxph.relsurv(formo,
                     data=msebmt, split.transitions = c(4,7,10,12), 
...
newdata <- msebmt %>% filter(id==1)
newdata$strata <- newdata$trans

mod <- msfit(cfull,newdata,trans=tmat)
mod$Haz %>% 
  group_by(trans) %>% 
  summarise(n=n())


mod2 <- msfit.coxph.relsurv(cx2,newdata,trans=tmat)
mod2$Haz %>% 
  group_by(trans) %>% 
  summarise(n=n())